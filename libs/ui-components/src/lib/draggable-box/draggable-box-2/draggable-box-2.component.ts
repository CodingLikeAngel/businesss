...
export const draggableBox2Variants = variants;
export type DraggableBox2VariantType = typeof draggableBox2Variants[number];

export interface DraggableBox2CustomStyles {
...
@Component({
  selector: 'lib-ui-components-draggable-box-2',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './draggable-box-2.component.html',
  styleUrl: './draggable-box-2.component.scss',
})
export class UIDraggableBox2Component implements AfterViewInit {
  variant = input<DraggableBox2VariantType>('secondary');
  rounded = input<'none' | 'md' | 'full'>('md');
  size = input<'sm' | 'md' | 'lg'>('md');
  dark = input<boolean>(false);
  content = input<string>('Drag me');
  customStyles = input<DraggableBox2CustomStyles>({});
