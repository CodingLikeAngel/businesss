...
// Variantes específicas del botón-1 (Minimalista)
export const button1SpecificVariants = ['icon-only', 'text-only', 'glass'] as const;

// Combinamos variantes globales y específicas
export const baseButton1Variants = [...globalVariants, ...button1SpecificVariants] as const;
type BaseButton1Variant = typeof baseButton1Variants[number];

// Interfaz para estilos personalizados
export interface Button1CustomStyles {
...
@Component({
  selector: 'lib-ui-components-button-1',
  standalone: true,
...
export class UIButton1Component {
  variant = input<BaseButton1Variant | string>('primary');
  ...
  customStyles = input<Button1CustomStyles>({});
  ...
  buttonClasses = computed(() => {
    const classes = ['btn-1', `btn-1-${this.variant()}`];
    if (this.rounded() !== 'none') classes.push(`btn-1-rounded-${this.rounded()}`);
    classes.push(`btn-1-${this.size()}`);
    if (this.dark()) classes.push('dark');
    if (this.disabled()) classes.push('btn-1-disabled');
