...
// Variantes específicas del botón-3 (Modern/Sleek)
export const button3SpecificVariants = ['icon-only', 'text-only', 'glass'] as const;

// Combinamos variantes globales y específicas
export const baseButton3Variants = [...globalVariants, ...button3SpecificVariants] as const;
type BaseButton3Variant = typeof baseButton3Variants[number];

// Interfaz para estilos personalizados
export interface Button3CustomStyles {
...
@Component({
  selector: 'lib-ui-components-button-3',
  standalone: true,
...
export class UIButton3Component {
  variant = input<BaseButton3Variant | string>('primary');
  ...
  customStyles = input<Button3CustomStyles>({});
  ...
  buttonClasses = computed(() => {
    const classes = ['btn-3', `btn-3-${this.variant()}`];
    if (this.rounded() !== 'none') classes.push(`btn-3-rounded-${this.rounded()}`);
    classes.push(`btn-3-${this.size()}`);
    if (this.dark()) classes.push('dark');
    if (this.disabled()) classes.push('btn-3-disabled');
