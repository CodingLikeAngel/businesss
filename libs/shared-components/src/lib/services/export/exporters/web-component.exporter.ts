...
export class WebComponentExporter extends ExporterBase {
  ...
  generate(component: ComponentMetadata): string {
    ...
    const baseClassName = component.baseClassName || 'component';
    const htmlTag = component.htmlTag || 'div';
...
class ${className} extends HTMLElement {
  ...
  constructor() {
    ...
    this._state = {
      ${props.length ? props.map(p => `${p.name}: ${this.formatPropValue(p.defaultValue)}`).join(',\n      ') : ''}
    };
  }
...
${props.length ? props.map(p => `  get ${p.name}() {
    return this._state.${p.name};
  }`).join('\n\n  ') : ''}

  // Setters for properties
${props.length ? props.map(p => `  set ${p.name}(value) {
    this._state.${p.name} = value;
    this.setAttribute('${p.name}', value);
  }`).join('\n\n  ') : ''}

  render() {
    const className = \`${baseClassName}\${this._state.variant ? ' ' + this._state.variant : ''}\`.trim();
    
    this.shadowRoot.innerHTML = \`
      <style>
        ${this.escapeTemplateLiterals((component.styles && component.styles.content) ? component.styles.content : '')}
      </style>
      <${htmlTag}
        class="\${className}"
    ...
    // Add event listeners
    const button = this.shadowRoot.querySelector('${htmlTag}');
    if (button) {
