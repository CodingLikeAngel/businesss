...
export abstract class BaseHomeFeatureComponent implements OnInit, OnDestroy {
  ...
  testimonialsConfig: TestimonialsConfig; // Nueva propiedad
  ...
  constructor(@Inject(PLATFORM_ID) protected platformId: object, protected variantService: VariantService) {
    this.isMobile = false;
    ...
    this.testimonialsConfig = this.variantService.getCurrentTestimonialsConfig(); // Inicializar
    ...
    this.configSubs.push(
      ...
      this.variantService.testimonialsConfig$.subscribe((config) => (this.testimonialsConfig = config)) // Nueva suscripción
    );
  }
  ...
  getVariant(componentId: string): CardVariant {
    ...
    const validVariants = [
      'default', 'cyberpunk', 'jungle', 'enchanted', 'mystic', 'ancient', 'twilight', 'frosty', 'desert', 'candy',
