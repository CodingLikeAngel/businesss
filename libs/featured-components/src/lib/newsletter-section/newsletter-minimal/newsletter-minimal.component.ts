...
type NewsletterMinimalVariantType = typeof newsletterMinimalVariants[number] | (string & {});

export interface NewsletterMinimalCustomStyles {
...
@Component({
  selector: 'lib-ui-newsletter-minimal',
  standalone: true,
  imports: [CommonModule, FormsModule, UIInputComponent, UIButtonComponent],
  templateUrl: './newsletter-minimal.component.html',
  styleUrls: ['./newsletter-minimal.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class UINewsletterMinimalComponent {
  title = input('Stay Updated');
  description = input('Get the latest updates delivered to your inbox.');
  placeholder = input('Enter your email');
  buttonText = input('Subscribe');
  variant = input<NewsletterMinimalVariantType>('primary');
  customStyles = input<NewsletterMinimalCustomStyles>({});
