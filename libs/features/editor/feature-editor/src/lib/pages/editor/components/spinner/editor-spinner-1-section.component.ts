...
import { UISpinner1Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-spinner-1-section',
  standalone: true,
  imports: [
    CommonModule,
    UISpinner1Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_spinner-1'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_spinner-1', section.content, 'spinner-1'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_spinner-1'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-spinner-1
          [variant]="$any(section.content['variant'] || 'secondary')"
          [size]="$any(section.content['size'] || 'md')"
          [dark]="section.content['dark'] || false"
          [speed]="$any(section.content['speed'] || 'normal')"
...
export class EditorSpinner1SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_spinner-1');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_spinner-1');
  }
}
