...
@Component({
      ...
      <div class="isolated-mode-container" (click)="$event.stopPropagation()">
        ...
        <div class="isolated-mode-header">
          <div class="header-breadcrumb">
            ...
            <span class="component-name">MÉTRICAS Y CONTADORES PRO</span>
          </div>
          ...
          <div class="controls-sidebar">
            <div class="sidebar-scroll-content">
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'items'">
                ...
                <div class="item-list">
                  <div *ngFor="let stat of editableItems; let i = index" 
                    ...
                    <div class="item-main">
                      ...
                      <div class="flex-1 overflow-hidden">
                        <div class="text-[10px] font-bold text-white uppercase">{{ stat.label || 'Métrica' }}</div>
                        <div class="text-[12px] text-indigo-400 font-black">{{ stat.value || '0' }}{{ stat.suffix || '' }}</div>
                      </div>
                    ...
                    <div *ngIf="selectedIndex === i" class="item-details animate-fade-in">
                       <div class="grid grid-cols-2 gap-2">
                          <div>
                             ...
                             <input type="text" [(ngModel)]="stat.label" class="premium-input-mini">
                          </div>
                          <div>
                             <label>Valor (Número)</label>
                             <input type="text" [(ngModel)]="stat.value" class="premium-input-mini">
                          </div>
                       </div>
                       <div class="grid grid-cols-2 gap-2 mt-3">
                          <div>
                             <label>Sufijo (%, +, k...)</label>
                             <input type="text" [(ngModel)]="stat.suffix" class="premium-input-mini">
                          </div>
                          <div>
                             <label>Icono</label>
                             <input type="text" [(ngModel)]="stat.icon" class="premium-input-mini" placeholder="👤, ⭐, 💰...">
                          </div>
                       </div>
                    </div>
                  </div>
                ...
                <button (click)="addItem()" class="add-btn-mini mt-4">+ Añadir Métrica</button>
              </div>
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'design'">
                <div class="section-header">
                  <span class="section-icon">🎨</span>
                  <h4>ESTILO DE CELDAS</h4>
                </div>
                
                <div class="control-group">
                  <label>Variante Visual</label>
                  <select [(ngModel)]="editableContent.variant" class="premium-input">
                    <option value="default">Glaseado (Glass)</option>
                    <option value="minimal">Minimalista</option>
                    <option value="bold">Enfatizado (Bold)</option>
                    <option value="outline">Contorneado</option>
                  </select>
                </div>
                ...
                <div class="control-row grid grid-cols-2 gap-2">
                   <div class="control-group">
                      ...
                      <input type="number" [(ngModel)]="editableContent.columns" min="1" max="6" class="premium-input">
                   </div>
                   <div class="control-group">
                      <label>Grosor Texto</label>
                      <select [(ngModel)]="editableContent.fontWeight" class="premium-input">
                        <option value="medium">Medio</option>
                        <option value="bold">Negrita</option>
                        <option value="black">Extra Negrita</option>
                      </select>
                   </div>
                </div>
          ...
          <div class="isolated-canvas">
            <div class="canvas-inner">
               <div class="draggable-wrapper"
                   [style.width.px]="editableStyles.maxWidth || 900">
            ...
            <div class="modern-position-dock">
               <div class="dock-item"><span class="label">MÉTRICAS</span><span class="value text-indigo-400">{{ editableItems.length }}</span></div>
               <div class="dock-divider"></div>
               <div class="dock-item"><span class="label">VAR</span><span class="value text-blue-400 capitalize">{{ editableContent.variant }}</span></div>
            </div>
          </div>
        </div>

        <div class="isolated-mode-footer">
          <div class="footer-hint">Stats Pro: Muestra el éxito de tu proyecto con datos impactantes.</div>
          <div class="footer-actions-btns">
            <button class="btn-clean secondary" (click)="cancel()">Descartar cambios</button>
            <button class="btn-clean primary" (click)="apply()">Publicar Métricas</button>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .stat-item-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 12px; cursor: pointer; transition: all 0.2s; }
    .stat-item-card:hover { border-color: rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); }
    .stat-item-card.active { border-color: #10b981; background: rgba(16, 185, 129, 0.05); }
...
export class EditorStatsIsolatedModeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.editableItems.length === 0) {
      this.editableItems = [
        { label: 'Proyectos', value: '150', suffix: '+', icon: '🚀' },
        { label: 'Clientes', value: '500', suffix: '+', icon: '👤' },
        { label: 'Años', value: '12', suffix: '', icon: '📅' },
        { label: 'Países', value: '25', suffix: '', icon: '🌍' }
      ];
    }
    this.editableStyles = { 
        ...this.config.styles,
