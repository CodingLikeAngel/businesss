...
import { UIChip1Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-chip-1-section',
  standalone: true,
  imports: [
    CommonModule,
    UIChip1Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_chip-1'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_chip-1', section.content, 'chip-1'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_chip-1'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-chip-1
...
export class EditorChip1SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_chip-1');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_chip-1');
  }
}
