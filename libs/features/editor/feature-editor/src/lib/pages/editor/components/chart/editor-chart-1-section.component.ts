...
import { UIChart1Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-chart-1-section',
  standalone: true,
  imports: [
    CommonModule,
    UIChart1Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_chart-1'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_chart-1', section.content, 'chart-1'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_chart-1'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-chart-1
...
export class EditorChart1SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_chart-1');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_chart-1');
  }
}
