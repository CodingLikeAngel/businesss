...
import { UISpinner2Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-spinner-2-section',
  standalone: true,
  imports: [
    CommonModule,
    UISpinner2Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_spinner-2'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_spinner-2', section.content, 'spinner-2'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_spinner-2'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-spinner-2
...
export class EditorSpinner2SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_spinner-2');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_spinner-2');
  }
}
