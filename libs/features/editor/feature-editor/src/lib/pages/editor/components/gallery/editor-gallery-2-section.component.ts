...
import { UIGallery2Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-gallery-2-section',
  standalone: true,
  imports: [
    CommonModule,
    UIGallery2Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_gallery-2'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_gallery-2', section.content, 'gallery-2'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_gallery-2'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-gallery-2
...
export class EditorGallery2SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_gallery-2');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_gallery-2');
  }
}
