...
import { UIAccordion2Component, UIAccordionComponent, UIAccordion1Component, UIAccordion3Component } from '@negocio/ui-components';
...
@Component({
  selector: 'lib-editor-accordion-2-section',
  standalone: true,
  imports: [
    CommonModule,
    ...
    <div #sectionElement
      ...
      <div class="absolute top-0 left-0 right-0 h-10 bg-white/80 backdrop-blur-sm border-b border-slate-200 flex items-center justify-between px-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Acordeón V2</span>
        </div>
        <button 
      ...
      <div #componentElement class="editor-element pt-8"
           [class.is-selected]="selectedElementId === section.id + '_accordion-2'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_accordion-2', section.content, 'accordion-2'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_accordion-2'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">

        <lib-ui-components-accordion-2
          *ngIf="section.content['accordionVariant'] === 'accordion-2' || !section.content['accordionVariant']"
        ...
        <lib-ui-components-accordion-3
          *ngIf="section.content['accordionVariant'] === 'accordion-3'"
          [variant]="$any(section.content['variant'] || 'secondary')"
          [rounded]="$any(section.content['rounded'] || 'md')"
          [size]="$any(section.content['size'] || 'md')"
          [dark]="section.content['dark'] || false"
...
export class EditorAccordion2SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_accordion-2');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_accordion-2');
  }

  @HostListener('window:keydown', ['$event'])
  handleKeyDown(event: KeyboardEvent) {
    if ((event.key === 'i' || event.key === 'I') && !event.ctrlKey && !event.metaKey) {
      if (this.selectedElementId === this.section.id + '_accordion-2') {
  ...
  openIsolatedMode(event?: MouseEvent) {
    ...
    this.isolatedConfig = {
      ...
      elementId: this.section.id + '_accordion-2',
