...
import { UITable2Component } from '@negocio/ui-components';
import { EnhancedBaseEditorSectionComponent } from '../enhanced-base-editor-section.component';

@Component({
  selector: 'lib-editor-table-2-section',
  standalone: true,
  imports: [
    CommonModule,
    UITable2Component,
  ...
  template: `
    <div #sectionElement
      ...
      <div #componentElement class="editor-element"
           [class.is-selected]="selectedElementId === section.id + '_table-2'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_table-2', section.content, 'table-2'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_table-2'}}"
           sectionId="{{section.id}}"
           (visualEvents)="handleComponentEvent($event)">
        <lib-ui-components-table-2
          ...
          [striped]="section.content['striped'] || false"
          [hoverable]="section.content['hoverable'] || false"
...
export class EditorTable2SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  @ViewChild('sectionElement', { static: true }) sectionElement!: ElementRef;
  @ViewChild('componentElement', { static: true }) componentElement!: ElementRef;

  ngAfterViewInit() {
    this.applySectionVisualEditing(this.sectionElement, this.section.id);
    this.applyElementVisualEditing(this.componentElement, this.section.id + '_table-2');
  }
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_table-2');
  }
}
