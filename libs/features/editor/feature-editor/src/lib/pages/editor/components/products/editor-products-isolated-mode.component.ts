...
@Component({
      ...
      <div class="isolated-mode-container" (click)="$event.stopPropagation()">
        ...
        <div class="isolated-mode-header">
          <div class="header-breadcrumb">
            ...
            <span class="component-name">E-COMMERCE CATALOG GOLD</span>
          </div>
        ...
        <div class="isolated-mode-body">
          ...
          <div class="controls-sidebar">
            <div class="sidebar-tabs">
              ...
              <button [class.active]="activeTab === 'design'" (click)="activeTab = 'design'">TIENDA</button>
            </div>
            ...
            <div class="sidebar-scroll-content">
                ...
                <div class="item-list">
                  <div *ngFor="let item of editableItems; let i = index" 
                    ...
                    <div class="item-main">
                      ...
                      <input type="text" [(ngModel)]="item.name" (ngModelChange)="onContentChange()" class="premium-input-mini" placeholder="Nombre...">
                    ...
                    <div *ngIf="selectedIndex === i" class="item-details animate-fade-in">
                        <div class="grid grid-cols-2 gap-2">
                           <div class="control-group-mini">
                              ...
                              <input type="text" [(ngModel)]="item.price" (ngModelChange)="onContentChange()" class="premium-input-mini" placeholder="Ej: 99.90">
                           </div>
                           <div class="control-group-mini">
                              <label>Moneda</label>
                              <input type="text" [(ngModel)]="item.currency" (ngModelChange)="onContentChange()" class="premium-input-mini" placeholder="€ / $">
                           </div>
                        </div>

                        <label class="mt-4">Imagen del Producto (URL)</label>
                        <input type="text" [(ngModel)]="item.image" (ngModelChange)="onContentChange()" class="premium-input-mini" placeholder="https://...">

                        <label class="mt-4">Categoría / Etiqueta</label>
                        <input type="text" [(ngModel)]="item.category" (ngModelChange)="onContentChange()" class="premium-input-mini" placeholder="Premium">
                    </div>
                  </div>
                </div>

                <button (click)="addItem()" class="add-btn-premium mt-4">+ Nuevo Producto</button>
              </div>
              ...
              <div class="sidebar-section animate-fade-in" *ngIf="activeTab === 'design'">
                <div class="section-header">
                  <span class="section-icon">🎨</span>
                  <h4>ESTILO DE EXPOSICIÓN</h4>
                </div>
                
                <div class="control-group">
                  <label>Configuración de Grid</label>
                  <select [(ngModel)]="editableContent.variant" (ngModelChange)="onContentChange()" class="premium-select">
                    ...
                    <option value="classic">Clásico (Bordes)</option>
                    <option value="luxury">Lujoso (Sombras)</option>
                    <option value="minimal">Ultra-Limpio</option>
                  </select>
                </div>
                ...
                <div class="control-row grid grid-cols-2 gap-3">
                   <div class="control-group">
                      ...
                      <input type="range" min="1" max="5" [(ngModel)]="editableContent.columns" (ngModelChange)="onContentChange()" class="premium-range">
                   </div>
                   <div class="control-group">
                      <label>Relación Aspecto</label>
                      <select [(ngModel)]="editableStyles.imageRatio" (ngModelChange)="onContentChange()" class="premium-select">
                ...
                <div class="control-group mt-6">
                  <label>Color de Enfoque (Precio/Cta)</label>
          ...
          <div class="isolated-canvas" #canvas (mousedown)="onCanvasMouseDown($event)">
            ...
            <div class="modern-position-dock">
              <div class="dock-item"><span class="label">STOCK</span><span class="value">{{ editableItems.length }}</span></div>
              <div class="dock-divider"></div>
              <div class="dock-item"><span class="label">GRID</span><span class="value uppercase text-blue-400">{{ editableContent.variant }}</span></div>
              <div class="dock-divider"></div>
              <div class="dock-item"><span class="label">CANVAS</span><span class="value">{{ currentSize.width }}px</span></div>
            </div>
          </div>
        </div>

        <div class="isolated-mode-footer">
          <div class="footer-hint">Products Gold: Optimiza tus imágenes para una carga un 50% más rápida.</div>
          <div class="footer-actions-btns">
            <button class="btn-clean secondary" (click)="cancel()">Descartar cambios</button>
            <button class="btn-clean primary" (click)="apply()">Actualizar Tienda</button>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .product-item-card { background: #0f172a; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 12px; cursor: pointer; transition: all 0.2s;
...
export class EditorProductsIsolatedModeComponent extends BaseIsolatedModeComponent {
  ...
  protected override initializeState() {
    ...
    this.editableStyles = { 
        ...
        imageRatio: this.config.styles.imageRatio || '1/1'
    };
    
    this.currentPosition = { ...(this.config.position || { x: 150, y: 100 }) };
    this.currentSize = { 
