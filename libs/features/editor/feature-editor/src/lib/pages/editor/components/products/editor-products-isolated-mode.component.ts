...
import { ProductsSectionComponent } from '@negocio/featured-components';
import { IsolatedModeConfig } from '../enhanced-visual-editing.interfaces';

@Component({
  selector: 'lib-editor-products-isolated-mode',
  standalone: true,
  imports: [CommonModule, FormsModule, ProductsSectionComponent],
  template: `
      ...
      <div class="isolated-mode-container" (click)="$event.stopPropagation()">
        ...
        <div class="isolated-mode-header">
          <div class="header-breadcrumb">
            <span class="mode-badge">🏷️ PRODUCTS EDITOR</span>
            <span class="separator">/</span>
            <span class="component-name">CATÁLOGO DE PRODUCTOS PRO</span>
          </div>
        ...
        <div class="isolated-mode-body">
          <div class="controls-sidebar">
            <div class="sidebar-scroll-content">
              ...
              <div class="sidebar-tabs">
                <button [class.active]="activeTab === 'items'" (click)="activeTab = 'items'">PRODUCTOS</button>
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'items'">
                <div class="section-header">
                  ...
                  <h4>INVENTARIO VISIBLE</h4>
                </div>
                
                <div class="item-list">
                  <div *ngFor="let product of editableItems; let i = index" 
                       class="product-item-card" 
                       [class.active]="selectedIndex === i"
                       (click)="selectedIndex = i">
                    <div class="item-main">
                      <span class="item-index">{{ i + 1 }}</span>
                      <input type="text" [(ngModel)]="product.name" class="premium-input-mini" placeholder="Nombre del producto">
                    ...
                    <div *ngIf="selectedIndex === i" class="item-details animate-fade-in">
                       <div class="grid grid-cols-2 gap-2">
                          <div>
                             <label>Precio</label>
                             <input type="text" [(ngModel)]="product.price" class="premium-input-mini" placeholder="Ej: 99.99">
                          </div>
                          <div>
                             <label>Moneda</label>
                             <input type="text" [(ngModel)]="product.currency" class="premium-input-mini" placeholder="€ / $">
                          </div>
                       </div>
                       
                       <label class="mt-3">Imagen URL</label>
                       <input type="text" [(ngModel)]="product.image" class="premium-input-mini" placeholder="https://...">

                       <label class="mt-3">Categoría</label>
                       <input type="text" [(ngModel)]="product.category" class="premium-input-mini" placeholder="Premium">
                    </div>
                  </div>
                </div>

                <button (click)="addItem()" class="add-btn-mini mt-4">+ Añadir Producto</button>
              </div>
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'design'">
                <div class="section-header">
                  <span class="section-icon">🎨</span>
                  <h4>ESTILO DE TIENDA</h4>
                </div>
                
                <div class="control-group">
                  <label>Variante de Grid</label>
                  <select [(ngModel)]="editableContent.variant" class="premium-input">
                    <option value="modern">Moderno (Hover)</option>
                    <option value="classic">Clásico (Fijo)</option>
                    <option value="minimal">Minimalista</option>
                    <option value="luxury">Lujoso (Dark)</option>
                  </select>
                </div>
                ...
                <div class="control-row grid grid-cols-2 gap-2">
                   <div class="control-group">
                      ...
                      <input type="number" [(ngModel)]="editableContent.columns" min="1" max="5" class="premium-input">
                   </div>
                   <div class="control-group">
                      <label>Aspect Ratio</label>
                      <select [(ngModel)]="editableStyles.imageRatio" class="premium-input">
          ...
          <div class="isolated-canvas">
            ...
            <div class="modern-position-dock">
               <div class="dock-item"><span class="label">PRODUCTOS</span><span class="value text-indigo-400">{{ editableItems.length }}</span></div>
               <div class="dock-divider"></div>
               <div class="dock-item"><span class="label">LAYOUT</span><span class="value text-blue-400 uppercase">{{ editableContent.variant }}</span></div>
            </div>
          </div>
        </div>

        <div class="isolated-mode-footer">
          <div class="footer-hint">Products Pro: Crea un catálogo visualmente atractivo y fácil de navegar.</div>
          <div class="footer-actions-btns">
            <button class="btn-clean secondary" (click)="cancel()">Descartar cambios</button>
            <button class="btn-clean primary" (click)="apply()">Actualizar Catálogo</button>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .product-item-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 12px; cursor: pointer; transition: all 0.2s; }
    .product-item-card:hover { border-color: rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); }
    .product-item-card.active { border-color: #10b981; background: rgba(16, 185, 129, 0.05); }
    ...
    .draggable-wrapper { position: relative; width: 100%; padding: 20px; border: 1.5px dashed #10b981; border-radius: 12px; background: rgba(255,255,255,0.01); }
...
export class EditorProductsIsolatedModeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.editableItems.length === 0) {
      this.editableItems = [
        { name: 'Producto Alpha', price: '49.99', currency: '€', image: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30', category: 'Premium' },
        { name: 'Producto Beta', price: '29.99', currency: '€', image: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e', category: 'Basic' }
      ];
    }
    this.editableStyles = { 
        ...this.config.styles,
