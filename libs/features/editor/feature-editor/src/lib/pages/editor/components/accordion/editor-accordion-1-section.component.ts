...
import { UIAccordion1Component, UIAccordionComponent, UIAccordion2Component, UIAccordion3Component } from '@negocio/ui-components';
...
@Component({
  selector: 'lib-editor-accordion-1-section',
  standalone: true,
  imports: [
    CommonModule,
    ...
    <div #sectionElement
      ...
      <div class="absolute top-0 left-0 right-0 h-10 bg-white/80 backdrop-blur-sm border-b border-slate-200 flex items-center justify-between px-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Acordeón V1</span>
        </div>
        <button 
      ...
      <div #componentElement class="editor-element pt-8"
           [class.is-selected]="selectedElementId === section.id + '_accordion-1'"
           (click)="selectElement($event, getMergedElement(section.id, section.id + '_accordion-1', section.content, 'accordion-1'))"
           [enhancedVisualEditable]="getComponentConfig()"
           elementId="{{section.id + '_accordion-1'}}"
           sectionId="{{section.id}}"
...
export class EditorAccordion1SectionComponent extends EnhancedBaseEditorSectionComponent implements AfterViewInit {
  ...
  handleComponentEvent(event: VisualEditingEvent): void {
    this.handleVisualEvent(event, this.section.id + '_accordion-1');
  }

  @HostListener('window:keydown', ['$event'])
  handleKeyDown(event: KeyboardEvent) {
    if ((event.key === 'i' || event.key === 'I') && !event.ctrlKey && !event.metaKey) {
      if (this.selectedElementId === this.section.id + '_accordion-1') {
  ...
  openIsolatedMode(event?: MouseEvent) {
    ...
    this.isolatedConfig = {
      ...
      elementId: this.section.id + '_accordion-1',
