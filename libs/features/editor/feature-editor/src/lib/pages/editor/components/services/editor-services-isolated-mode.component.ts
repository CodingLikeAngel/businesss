...
@Component({
      ...
      <div class="isolated-mode-container" (click)="$event.stopPropagation()">
        ...
        <div class="isolated-mode-header">
          <div class="header-breadcrumb">
            ...
            <span class="component-name">PORTAFOLIO DE SERVICIOS PRO</span>
          </div>
          ...
          <div class="controls-sidebar">
            <div class="sidebar-scroll-content">
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'items'">
                ...
                <div class="item-list">
                  <div *ngFor="let service of editableItems; let i = index" 
                       class="service-item-card" 
                       [class.active]="selectedIndex === i"
                       (click)="selectedIndex = i">
                    <div class="item-main">
                      <span class="item-index">{{ i + 1 }}</span>
                      <input type="text" [(ngModel)]="service.title" class="premium-input-mini" placeholder="Ej: Consultoría IT">
                      <button (click)="removeItem(i, $event)" class="delete-btn">✕</button>
                    </div>
                    <div *ngIf="selectedIndex === i" class="item-details animate-fade-in">
                       <label>Descripción</label>
                       <textarea [(ngModel)]="service.description" class="premium-input-mini h-24 mt-1" placeholder="Describe este servicio..."></textarea>
                       
                       <div class="grid grid-cols-2 gap-2 mt-3">
                          <div>
                             <label>Icono</label>
                             <input type="text" [(ngModel)]="service.icon" class="premium-input-mini" placeholder="⚡, 🛡️, ⚙️...">
                          </div>
                          <div>
                             <label>Enlace/Botón</label>
                             <input type="text" [(ngModel)]="service.linkText" class="premium-input-mini" placeholder="Saber más">
                          </div>
                       </div>
                    </div>
                  </div>
                ...
                <button (click)="addItem()" class="add-btn-mini mt-4">+ Añadir Servicio</button>
              </div>
              ...
              <div class="sidebar-section" *ngIf="activeTab === 'design'">
                ...
                <div class="control-group">
                  ...
                  <select [(ngModel)]="editableContent.variant" class="premium-input">
                    ...
                    <option value="cards">Tarjetas Elevadas</option>
                ...
                <div class="control-row grid grid-cols-2 gap-2">
                   <div class="control-group">
                      ...
                      <input type="number" [(ngModel)]="editableContent.columns" min="1" max="4" class="premium-input">
                   </div>
                   <div class="control-group">
                      <label>Radio Borde</label>
                      <select [(ngModel)]="editableStyles.borderRadius" class="premium-input">
                        <option value="0px">Recto</option>
                        <option value="12px">Suave</option>
                        <option value="24px">Redondeado</option>
                        <option value="50px">Cápsula</option>
                      </select>
                   </div>
                </div>
          ...
          <div class="isolated-canvas">
            <div class="canvas-inner">
               <div class="draggable-wrapper"
                   [style.width.px]="editableStyles.maxWidth || 1000">
            ...
            <div class="modern-position-dock">
               <div class="dock-item"><span class="label">SERVICIOS</span><span class="value text-indigo-400">{{ editableItems.length }}</span></div>
               <div class="dock-divider"></div>
               <div class="dock-item"><span class="label">GRID</span><span class="value text-blue-400 capitalize">{{ editableContent.variant }}</span></div>
            </div>
          </div>
        </div>

        <div class="isolated-mode-footer">
          <div class="footer-hint">Services Pro: Proyecta confianza y profesionalismo con tus ofertas principales.</div>
  ...
  styles: [`
    ...
    .service-item-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 12px; cursor: pointer; transition: all 0.2s; }
    .service-item-card:hover { border-color: rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); }
    .service-item-card.active { border-color: #10b981; background: rgba(16, 185, 129, 0.05); }
...
export class EditorServicesIsolatedModeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.editableItems.length === 0) {
      this.editableItems = [
        { title: 'Servicio Web', description: 'Desarrollo de sitios modernos', icon: '💻', linkText: 'Ver más' },
        { title: 'Marketing Digital', description: 'Estrategias de crecimiento', icon: '📈', linkText: 'Ver más' }
      ];
    }
    this.editableStyles = { 
        ...this.config.styles,
